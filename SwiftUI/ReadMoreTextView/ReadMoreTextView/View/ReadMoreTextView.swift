...
struct ReadMoreTextView: View {
    ...
    init(_ text: String, lineLimit: Int, font: UIFont = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.body)) {
        ...
        self.font = font
        self._shrinkText = State(wrappedValue: text)
    }
    
    var body: some View {
        ZStack(alignment: .bottomLeading) {
                    Group {
                        Text(self.expanded ? text : shrinkText) + Text(moreLessText).bold().foregroundColor(.blue)
                    }
                    .animation(.easeInOut)
...
struct ContentView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        VStack(alignment: .leading, spacing: 10) {
            ReadMoreTextView("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Sed ut laborum", lineLimit: 6)

            ReadMoreTextView("Small text", lineLimit: 3)
                .foregroundColor(.red)

            ReadMoreTextView("Render the limited text and measure its size, R", lineLimit: 1)
                .foregroundColor(.green)

            ReadMoreTextView("Create a ZStack with unbounded height to allow the inner Text as much, Render the limited text and measure its size, Hide the background Indicates whether the text has been truncated in its display.", lineLimit: 3)
